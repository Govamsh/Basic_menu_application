...
cserver()
{
	...
	cchoice=popupmenuv(cursor,11,1,0,"SFTBLEWRUDN",1);
...
fsrever()
{
	...
	char fname[30];
	fchoice=popupmenuv(filename9,1,23,"LPNSAMCOT",3);
...
char *str1,*str2;
{
	...
	menu(13,24,14,30,32,HALF_SHADOW);
...
check_saved()
{
	...
	if(saved==NO)
	{
		ans=messages("is not saved...","Save?");
...
del_line()
{
	...
	memove(currow,temp,endloc-temp);
...
char ch;
{
	...
	{
		if(exceed_size((unsigned)(ens=dloc-startloc+1)))
			return;

		memmove(temp+!,temp,endloc-temp);
